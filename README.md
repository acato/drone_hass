...
| **Lavagna, Liguria, IT** | EU — ENAC Specific category. **Primary path: PDRA-S02** (no C-class marking required, pilot-in-loop). **Fallback: full SORA operational authorisation** (for operational models outside PDRA-S02 envelope, e.g. truly unattended autonomy). | Pressure-tested; deferred deployment |
...
- **Alarm-to-airborne in under 30 seconds.** HA automation handles safety checks, dock opens, drone launches, video streams — **supervised-autonomy under Part 108 / EU PDRA-S02** (alarm-triggered mission, Flight Coordinator / airspace observer on duty retaining intervention capability; no per-flight human tap), or one-tap RPIC authorisation under Part 107. **Fully autonomous (no human on duty during flight) only where the governing authorisation expressly permits that operating model** — e.g., a future Part 108 approval that expressly covers unattended ops, or a bespoke EU Specific / SORA authorisation. PDRA-S02 does NOT cover unattended autonomy; it is a pilot-in-loop predefined-risk path.
