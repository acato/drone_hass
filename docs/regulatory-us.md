...
| Is fully autonomous alarm-triggered flight legal now? | **Not as an ordinary Part 107 operation.** The practical product path is **Part 108 once finalised**. Extraordinary Part 107 waiver-based pathways (under §107.31 / §107.33 and related) may exist, but they are fact-specific, difficult, and **not a reliable baseline for a general deployment model** — FAA has granted individual waivers but they are non-transferable and case-by-case. Treat waiver-dependence as off-path for product design. |
| Expected Part 108 availability | **Current planning assumption, not legal status.** NPRM published 2025-08-07; if the FAA stays near the NPRM trajectory, a final rule could arrive on the "summer 2026" general timeline — but **timing and substance remain uncertain until promulgation**. Verify publication status before relying on any Part-108-based compliance claim. |
| Operating tier likely to apply *(NPRM forecast, not settled law)* | *Under the NPRM as drafted*, **Part 108 Operating Permit** (not Certificate) — lower-risk operations, population density Categories 1–3 as proposed. Final rule may redefine tiers, density categories, or operations eligible for each. |
