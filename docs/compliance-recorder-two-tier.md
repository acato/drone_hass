...
**This is incompatible with GDPR** (`regulatory-eu.md` §8) and with any retention regime that requires footage to be deleted on a schedule or on data-subject request. The Italian Garante's general baseline on **fixed CCTV videosurveillance** (*Provvedimento in materia di videosorveglianza, 8 aprile 2010* [doc. web n. 1712680]) establishes a short default retention window (typically 24 h), extensible with justification (commonly cited cap of around 7 days), longer only for incident-linked footage. Extending that to drone footage is a **DPIA working assumption by analogy**, not a drone-specific published rule — see `regulatory-eu-it.md §6.2`. Either way, "never delete" is a GDPR violation, not a feature, and the two-tier recorder must support scheduled deletion.
