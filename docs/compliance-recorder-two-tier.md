...
**This is incompatible with GDPR** (`regulatory-eu.md` §8) and with any retention regime that requires footage to be deleted on a schedule or on data-subject request. The Italian Garante's general baseline on **fixed CCTV videosurveillance** (*Provvedimento 8 aprile 2010*) establishes a short default retention window (typically 24 h), extensible with justification (commonly cited cap of around 7 days), longer only for incident-linked footage. Extending that to drone footage is a **DPIA working assumption by analogy**, not a drone-specific published rule — see `regulatory-eu-it.md §6.2`, which also corrects a prior citation error (earlier drafts cited "Provvedimento 11 ottobre 2018" — that was a mis-cited site-specific decision, not the general baseline). Either way, "never delete" is a GDPR violation, not a feature, and the two-tier recorder must support scheduled deletion.
